 ...
 * preview, replacing the Python cv2.rectangle / putText / imshow calls.
 */
class OverlayView @JvmOverloads constructor(
    ...
    fun update(result: FrameResult, sourceWidth: Int, sourceHeight: Int, processingMs: Long) {
        this.result = result
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (person in frameResult.persons) {
            val paint = if (person.identified) boxPaintKnown else boxPaintUnknown
        ...
        val info = "FPS: %.1f | %dms | Known: %d | Tracked: %d".format(
            fps, processingMs, frameResult.knownDatabaseSize, frameResult.persons.size
        )
